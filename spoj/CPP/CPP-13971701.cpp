...
long double strip_small(point *strip, int n, long double d){
	...
	int i,j;
	for(i = 0; i < n; i++) {
	    for(j = i+1; j < n && abs(strip[j].y - strip[i].y) < mini; j++){
	    	long double temp = dis(strip[i] , strip[j]);
	        if(temp < d){
	            mini = temp;
	        }
	    }
...
long double bruteforce(point *px, int n){
	...
	for(i = 0; i < n; i++){
	    for( j = i+1; j < n; j++){
	    	long double temp = dis(px[i], px[j]);
	        if(temp < mini)
	            mini = temp;
	    }
	}
...
long double small(point *px, point *py, int n) {
	...
	long double dl = small(px, py, mid);
	long double dr = small(px+mid, py, n-mid);
