...
double Distance (pi a, pi b) {
        ...
        double dis = (double)((x2-x1)*(x2-x1)*1.0) + (double)((y2-y1)*(y2-y1)*1.0);
        dis = (double)sqrt((double)(dis*1.0));
...
double stripCheck (pi strip[], int size, double minDistTillNow) {
...
double closestpair(pi ptx[], pi pty[], int n) {
