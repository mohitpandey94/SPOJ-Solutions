...
long double Distance (pi a, pi b) {
        ...
        long double dis = (long double)((x2-x1)*(x2-x1)*1.0) + (long double)((y2-y1)*(y2-y1)*1.0);
...
long double stripCheck (pi strip[], int size, long double minDistTillNow) {
        ...
        repu(i,0,size,1) {
                ...
                for (j = i+1; j < size && abs(strip[j].second - strip[i].second) < finalMinDist; j++) {
...
long double closestpair(pi ptx[], pi pty[], int n) {
        ...
        long double minDistRight = closestpair(ptx+mid,PtsRightOfLine,n-mid);
