...
//
int tree[4*Sz]; bool lazy[4*Sz];

void update (int ss, int es, int ci, int us, int ue) {
    //cout<<ss<<" "<<es<<" "<<ci<<",\n";
    if (lazy[ci] != 0) {
        tree[ci] = es-ss+1 - tree[ci];
        if (ss != es) {
            //tree[ci] += lazy[ci]*(es-ss+1);
            lazy[2*ci] = !lazy[2*ci];
            lazy[2*ci+1] = !lazy[2*ci+1];
        }
        lazy[ci] = 0;
    }
    ...
    update(ss,(ss+es)/2,2*ci,us,ue);
    update ((ss+es)/2+1,es,2*ci+1,us,ue);

    tree[ci] = tree[2*ci]+tree[2*ci+1];
}
int query (int ss, int es, int ci, int qs, int qe) {
